...
class AttnEntropyTextAVGMergeKVCache_LayerWise:
    def __init__(
        self,
        hh_size=4,
        recent_size=512,
        ...
        print(f"Before DynamicCache-LayerWise:{recent_ratio}, {hh_ratio}")

    def __call__(self, past_key_values, attn_score_cache, layer_hh_ratio, layer_recent_ratio):

    
        # if self.hh_ratio is not None and attn_score_cache.shape[-2]>1:
        if self.hh_ratio is not None and layer_hh_ratio != 0:
        ...
        ######## add a new judgement here ######################

        if layer_hh_ratio == 0 or layer_recent_ratio == 0:
        ...
        # hh-selection
        bsz, num_heads, curr_seq_len, head_dim = past_key_values[0].shape # [1, 32, seq_len, 128]
        #################################only-image######################################
        ...
        text_len = curr_seq_len - self.image_position.shape[0] * 576

        # breakpoint()

        if self.recent_size < text_len:
            self.recent_size = text_len + 100  # keep text tokens
...
class AttnEntropyTextPivotMerge_LayerWise:
    def __init__(
        self,
        hh_size=4,
        recent_size=512,
    ...
    def __call__(self, past_key_values, attn_score_cache, layer_hh_ratio, layer_recent_ratio):
        ...
        if self.recent_size < text_len:
            self.recent_size = text_len # keep text tokens
...
class DynamicLlamaAttention_drop(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: LlamaConfig, layer_idx=None):
        ...
        self.kv_cache = AttnEntropyTextPivotMerge_LayerWise(   # AttnEntropyTextPivotMerge_LayerWise  # AttnEntropyTextAVGMergeKVCache_LayerWise
            hh_size=config.hh_size,
            recent_size=config.recent_size,
            k_seq_dim=2,
            v_seq_dim=2,
    ...
    def forward(
        ...
        if q_len != 1: # make sure in prompt encoding ################
            ...
            self.compute_multimodal_entropy(q_len, attn_weights)
        ...
        if self.layer_idx == 5:
            ...
            print('self layer ratio: ', self.layer_recent_ratio)
            print('pastkey value  shape', past_key_value[0].shape)
            print('key_states.shape[-2] - self.prfill_size == 1', key_states.shape[-2] - self.prfill_size == 1)
            print('self.prfill_size', self.prefill_score.shape[-1])


        if self.layer_hh_ratio != 0 and self.kv_cache.image_position.shape[0] > 0 and (key_states.shape[-2] - self.prfill_size == 1):  # not related to Q
            ...
            past_key_value = self.kv_cache(past_key_value, attention_score, self.layer_hh_ratio, self.layer_recent_ratio )
    ...
    def compute_multimodal_entropy(self, q_len, attn_weights):
        attn_weights_draw =  attn_weights.detach().clone()
        epsilon = 1e-12  # Small constant to prevent log(0)
        A_TV_clamped = attn_weights_draw.clamp(min=epsilon)
        attn_entropy = -torch.sum(A_TV_clamped * torch.log(A_TV_clamped), dim=(-2, -1))  # Sum over L_T and L_V

            # attn_entropy = math.fabs(attn_entropy)
        mean_attn_entropy = attn_entropy[0].mean()/q_len

        self.multimodal_entropy = mean_attn_entropy


    def compute_crossmodal_entropy(self, Q, K, text_indices, visual_indices):

        # Ensure indices are torch tensors and move them to the appropriate device
        device = Q.device
        if not torch.is_tensor(text_indices):
            text_indices = torch.tensor(text_indices, dtype=torch.long, device=device)
        if not torch.is_tensor(visual_indices):
            visual_indices = torch.tensor(visual_indices, dtype=torch.long, device=device)
        
        batch_size, num_heads, seq_length, dim = Q.shape

        Q_T = Q[:, :, text_indices, :]  # Text queries
        K_T = K[:, :, text_indices, :]  # Text keys
        Q_V = Q[:, :, visual_indices, :]  # Visual queries
        K_V = K[:, :, visual_indices, :]  # Visual keys

        scaling_factor = math.sqrt(dim)  
        scores_TV = torch.matmul(Q_T, K_V.transpose(-2, -1)) / scaling_factor
        A_TV = F.softmax(scores_TV, dim=-1, dtype=torch.float32).to(Q.dtype)  # Softmax over the last dimension (L_V)
        

        scores_VT = torch.matmul(Q_V, K_T.transpose(-2, -1)) / scaling_factor
        A_VT = F.softmax(scores_VT, dim=-1, dtype=torch.float32).to(Q.dtype) # Softmax over the last dimension (L_T)
        
        # Compute entropy for A_TV
        epsilon = 1e-12  # Small constant to prevent log(0)
        A_TV_clamped = A_TV.clamp(min=epsilon)
        entropy_TV = -torch.sum(A_TV_clamped * torch.log(A_TV_clamped), dim=(-2, -1))  # Sum over L_T and L_V
        
        # Compute entropy for A_VT
        A_VT_clamped = A_VT.clamp(min=epsilon)
        entropy_VT = -torch.sum(A_VT_clamped * torch.log(A_VT_clamped), dim=(-2, -1))

        text_len = len(text_indices)
        visual_len = len(visual_indices)
        mean_entropy_TV = entropy_TV[0].mean()
        mean_entropy_VT = entropy_VT[0].mean()

        cross_modal_entropy = mean_entropy_TV / text_len  + mean_entropy_VT / visual_len # Shape: (batch_size, num_heads)

        self.multimodal_entropy = cross_modal_entropy
...
class AttnEntropyTextAVGMergeLlamaAttention_drop(DynamicLlamaAttention_drop):
    def __init__(self, config: LlamaConfig, layer_idx=0):
        super().__init__(config, layer_idx)

        self.kv_cache = AttnEntropyTextAVGMergeKVCache_LayerWise(
            hh_size=config.hh_size,
            recent_size=config.recent_size,
            k_seq_dim=2,
            v_seq_dim=2,
