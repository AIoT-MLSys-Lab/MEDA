...
from ..kv_token_merge.modify_llama import H2OLlamaAttention_drop, \
                                            ...
                                            AttnEntropyTextAVGMergeLlamaAttention_drop
...
## test for dynamic mutlimodal cache allocation 
from ..kv_token_merge.v433_modeling_dynamic_llama import LlamaModel, LlamaForCausalLM
...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config, kv_mode, hh_ratio, recent_ratio):
        ...
        if kv_mode == "attn_entropy_TextAVGMerge":
            for layer_idx in range(num_layers):
                if layer_idx == 0:
                    config.hh_ratio = 0.1
                    config.recent_ratio = 0.1
                    self.model.layers[layer_idx].self_attn = AttnEntropyTextAVGMergeLlamaAttention_drop(config, layer_idx=layer_idx)
                elif layer_idx == 4:
                    config.hh_ratio = 0.1
                    config.recent_ratio = 0.1
                    self.model.layers[layer_idx].self_attn = AttnEntropyTextAVGMergeLlamaAttention_drop(config, layer_idx=layer_idx)
    ...
    def generate(
        ...
        **kwargs,
        ...
        # img_end_pos = img_st_pos + torch.tensor(576) #The end is img token  # 576 is the image token nums
